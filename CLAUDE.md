...
- **Serenaは使用しない**（コードがまだ存在しないため）

#### 実装準備時
1. `mcp__serena__list_dir` → プロジェクト構造の把握
2. `mcp__serena__get_symbols_overview` → ファイル内の象徴一覧
3. `/kiro:spec-impl <feature> [task-numbers]` → TDDによる実装開始
