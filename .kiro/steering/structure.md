...
**⚠️ 重要**: 本ドキュメントに記載されているディレクトリ構造は、プロジェクトの**目標構造（Target Structure）**です。現在はKiro仕様駆動開発の**仕様フェーズ**にあり、実装コードベースはまだ作成されていません。

### 現在存在するディレクトリ（Current State）
...
### 未作成のディレクトリ（実装開始後に作成予定）

```
meeting-minutes-automator/
├── src-tauri/                       # 🔵 Tauriアプリケーションコア（未作成）
├── src/                             # 🔵 フロントエンド（React）（未作成）
├── chrome-extension/                # 🔵 Chrome拡張機能（未作成）
├── python-stt/                      # 🔵 Pythonサイドカー（未作成）
└── tests/                           # 🔵 テストスイート（未作成）
```
...
2. **Phase 2: Specification** 🔵 進行中
   - meeting-minutes-core (MVP0): Design Validated → Tasks生成済み
   - meeting-minutes-stt (MVP1): Design Validated → Tasks生成準備完了
   - meeting-minutes-docs-sync (MVP2): Design Generated → レビュー待ち

3. **Phase 3: Implementation** ⚪ 未開始
   - Walking Skeleton実装（MVP0）
   - 実ディレクトリ構造の作成
   - 依存関係ファイル（Cargo.toml, package.json, requirements.txt）の作成
